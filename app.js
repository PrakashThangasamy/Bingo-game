...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createTable(){
        for(let tz=0;tz<noOfOps;tz++){
            ...
            let arr = shuffle(Array.from({length: tableSize*tableSize}, (v, k) => k+1)) 
            let iter = 0
    ...
    createTable()
    bingoDisplay()
    generatedNumbers = shuffle(Array.from({length: tableSize*tableSize}, (v, k) => k+1)) 
})
...
function checkStraight(table){
    let bingoCount = 0, d1Count=0, d2Count=0
    for(let i=0;i<tableSize;i++){
        ...
        for(j=0;j<tableSize;j++){
            ...
            if(i==j && checkElement(table,i,j)){
                d1Count++
            }
            if(i+j==tableSize-1 && checkElement(table,i,j)){
                d2Count++
            }
        }
        if(hCount>=tableSize){
